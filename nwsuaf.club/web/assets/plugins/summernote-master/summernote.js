    ...
    lang: {
      'zh-CN': {
        ...
        image: {
          ...
          shapeNone: '形状：无',
          dragImageHere: '将图片拖拽到这里',
          dropImage: '拖拽图片',
          selectFromFiles: '从文件中选择',
          maximumFileSize: '最大的文件大小',
          maximumFileSizeError: '文件大小达到上限',
          url: '图片的URL地址',
          remove: '移除图片'
        },
        link: {
        ...
        color: {
          ...
          setTransparent: '设为透明',
          reset: '重设',
          resetToDefault: '重设为默认'
        },
        shortcut: {
          shortcuts: '快捷键',
          close: '关闭',
          textFormatting: '格式化文本',
          action: '事件',
          paragraphFormatting: '格式化段落',
          documentStyle: '文档样式'
        },
        history: {
          undo: '撤销',
...
//                   '</div>';
            var body = '<p>' +
                     '<label>' + lang.image.url + '</label>' +
                     '<input class="note-image-url" type="url" />' +
                   '</p><p><label>' + lang.image.selectFromFiles + '</label>' +
                     '<input class="note-image-input" type="file" name="files" accept="image/*" />' +
                     imageLimitation +
                   '</p>';
...
//                   );
                    var body = '<p>' +
                     '<label>' + lang.link.textToDisplay + '</label>' +
                     '<input class="note-link-text" type="text" width="240px" />' +
                   '</p>' +
                   '<p>' +
                     '<label>' + lang.link.url + '</label>' +
                     '<input class="note-link-url" type="url" width="240px" />' +
                   '</p' +
                   (!options.disableLinkTarget ?
                     '<p>' +
                       '<label>' + '<input type="checkbox" checked> ' +
      ...
      help: function (lang, options) {
        var body = '<a class="modal-close pull-right" aria-hidden="true" tabindex="-1">' + lang.shortcut.close + '</a>' +
                   ...
                   '<p class="text-center">' +
                     ...
                     '<a href="//zxy97.com/" target="_blank">zxy97.com</a>' +
                   '</p>';
